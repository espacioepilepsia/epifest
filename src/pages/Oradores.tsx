        ...
        <div className="container mx-auto max-w-5xl relative z-10">
          ...
          >
            ...
            <p className="text-foreground/60 text-lg max-w-xl">
              Médicos, especialistas y voces de la comunidad que compartirán su experiencia
              el 26 y 27 de marzo de 2026.
            </p>
          </motion.div>
        </div>
      </div>
      ...
      <section ref={ref} className="px-4 pb-28">
        <div className="container mx-auto max-w-5xl">
          ...
          {!loading && speakers.length === 0 && (
            <p className="text-foreground/50 text-center py-24 text-lg">
              Los oradores se anunciarán próximamente 💜
            </p>
          )}
              ...
              {speakers.map((speaker, i) => (
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 right-0 p-4">
                    ...
                    {speaker.title && (
                      <p className="text-accent text-xs mt-0.5 font-medium">{speaker.title}</p>
                    )}
                    {speaker.institution && (
                      <p className="text-foreground/50 text-[10px] mt-0.5">{speaker.institution}</p>
                    )}
                    {speaker.bio && (
                      <span className="mt-2 text-[10px] text-secondary font-semibold opacity-0 group-hover:opacity-100 transition-opacity">
                        Ver perfil →
                      </span>
                    )}
                  </div>
      ...
      {selected && (
        ...
        >
          ...
          >
            <button
              onClick={() => setSelected(null)}
              className="absolute top-4 right-4 text-foreground/50 hover:text-foreground text-xl font-bold"
            >
              ✕
            </button>
            {selected.photo_url ? (
              <img
                ...
                alt={selected.name}
                className="w-28 h-28 rounded-full object-cover mx-auto mb-4 border-2 border-accent"
              />
            ) : (
            ...
            {selected.title && (
              <p className="text-accent text-sm text-center mt-1">{selected.title}</p>
            )}
            {selected.institution && (
              <p className="text-foreground/50 text-xs text-center mt-1">{selected.institution}</p>
            )}
            {selected.bio && (
              <p className="text-foreground/70 text-sm mt-4 leading-relaxed text-center">
                {selected.bio}
              </p>
            )}
          </motion.div>
