...
type TabType = 'general' | 'institution';
...
const ContactSection = () => {
  ...
  const [fromPicnic, setFromPicnic] = useState(false);
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (activeTab === 'institution') {
      extra_data = { sitio_web: (fd.get('sitio_web') as string) || '' };
    ...
    const { error: err } = await supabase.from('contact_messages').insert({
      type: activeTab,
      name, email, organization, message, extra_data,
    });
    ...
    await sendEmailCopy({ tab: activeTab, name, email, organization: organization ?? undefined, asunto: asuntoVal ?? undefined, message, extra: extra_data ?? undefined });
      ...
      <div className="container mx-auto max-w-2xl">
        ...
        <div className="flex flex-wrap gap-2 mb-8 justify-center">
          {tabs.map((tab) => (
            <button
              ...
              className={`rounded-full px-5 py-2 text-sm font-medium transition-all ${activeTab === tab.key ? 'bg-secondary text-secondary-foreground' : 'glass-card'}`}
            >
              {tab.label}
            </button>
          ))}
        ...
        {success ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="glass-card rounded-2xl p-10 text-center">
            ...
            <p className="text-foreground/70 text-base">Será contactado/a a la brevedad. 💜</p>
            <button onClick={resetForm} className="mt-6 text-sm text-accent underline hover:opacity-80 transition-opacity">Enviar otro mensaje</button>
          </motion.div>
        ) : (
          <form onSubmit={handleSubmit} className="glass-card rounded-2xl p-6 md:p-8 space-y-4">
            ...
            {activeTab === 'institution' ? (
              ...
              <>
                ...
                {/* Campo CABA — si asunto es picnic (por cualquier vía) */}
                ...
                {/* Mensaje no-CABA */}
                {noCaba ? (
                  <motion.div initial={{ opacity: 0, scale: 0.97 }} animate={{ opacity: 1, scale: 1 }} className="glass-card rounded-xl p-5 flex gap-3 items-start border border-accent/30">
